...
def canny(
    ...
    dtype = input.dtype
        ...
        #### auto low, high thres
    blurred3: Tensor = gaussian_blur2d(input, (3, 3), (1, 1))  # torch.Size([128, 3, 32, 32])
    # image = cv2.GaussianBlur(image, (3, 3), 0)

    torch.use_deterministic_algorithms(False)
    median, _ = torch.median(blurred3.view(blurred3.shape[0], -1), dim=1)
    lower_min = torch.min(blurred3.view(blurred3.shape[0], -1))
    upper_max = torch.max(blurred3.view(blurred3.shape[0], -1))
    # median = torch.median(blurred3.view(blurred3.shape[0], -1))

    lower = (1.0 - 0.33) * median
    upper = (1.0 + 0.33) * median

    mask = lower > upper
    lower_temp = lower.clone()
    lower[mask] = upper[mask]
    upper[mask] = lower_temp[mask]

    lower[lower_min >= lower] = lower_min
    upper[upper_max <= upper] = upper_max
    ...
    ####
    low_threshold = low_threshold.view(-1, 1, 1, 1)  # torch.Size([batch, 1, 1, 1])
    high_threshold = high_threshold.view(-1, 1, 1, 1)  # torch.Size([batch, 1, 1, 1])
    
    # edges: Tensor = torch.where(magnitude > low_threshold, magnitude, torch.tensor(0.0).float().cuda())
    edges: Tensor = torch.where(magnitude > low_threshold, magnitude, torch.tensor(0.0).half().cuda())
    ####
