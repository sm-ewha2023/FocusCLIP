...
from focusclip.models import MODELS
from focusclip.models.focusclip import FocusCLIP
from focusclip.utils.logging import get_logger
...
class Runner(pl.LightningModule):
    ...
    # Model Forward
    def forward(self, images, stain):
        return self.module(images, stain)
    ...
    def run_step(self, batch, batch_idx):
        images, labels, stains = batch

        logits, *_ = self.module(images, stains)

        losses = self.compute_losses(logits, labels)
        loss = sum([weight * losses[k] for k, weight in self.loss_weights.items()])

        metrics_exp = self.compute_per_example_metrics(logits, labels, "exp")
        metrics_max = self.compute_per_example_metrics(logits, labels, "max")
        metrics_top_2 = self.compute_topk_per_example_metrics(logits, labels, "top2")
        metrics_top_3 = self.compute_topk_per_example_metrics(logits, labels, "top3")
        metrics_top_5 = self.compute_topk_per_example_metrics(logits, labels, "top5")

        return {"loss": loss, **losses, **metrics_exp, **metrics_max, **metrics_top_2, **metrics_top_3, **metrics_top_5}
    ...
    def compute_topk_per_example_metrics(self, logits, y, gather_type="top2"):
        dtype = logits.dtype

        # predicted y of top-k
        predict_y = self.convert_logits_to_predicions(logits, gather_type)
        ...
        for i in range(len(y)):
            if y[i] in predict_y[i]:
                topk_correct += 1

        acc = torch.tensor(topk_correct / len(y)).type(logits.dtype)

        # print(f'predicted_y: {predict_y}')
        # print(f'y: {y}')

        return {f"acc_{gather_type}_metric": acc, "predict_y": predict_y}
