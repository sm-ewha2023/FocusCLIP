...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_num_gpus_used_in_parallel", type=int, default=2)
    ...
    for current_job_id, config_path in enumerate(tqdm(all_configs)):
        ...
        ####
        cli = f"CUDA_VISIBLE_DEVICES={current_gpu_id} python scripts/run.py --config {str(config_path)} {'--test_only' if args.test_only else ''} {'--ckpt_path' if args.ckpt_path else ''}"
        ####
